...
`RealMessageModeTest` checks real touch tabs, X/A switching, controller release, Activity recreation, and a failed history lookup retaining Messages. Its invalid Codex executable exists only as a temporary in-memory test value and is restored in `finally`. It does not send terminal input or create an agent turn. Compact-layout checks cover both mode labels as well as the hint strip.
