...
object ControllerCommands {
    ...
    fun forScreen(screen: Screen,mode: InputMode,acquiring: Boolean=false,review: Boolean=false,voicePhase: VoicePhase=VoicePhase.IDLE): List<ControllerCommand> {
        ...
        val voice=screen==Screen.VOICE
        val confirm=when { acquiring->"Wait";compose->if(review)"Send + Enter"else "Review";assistant->"Ask / choose";home->"Open";terminal->if(remote)"Enter once"else "Input";voice->when(voicePhase) { VoicePhase.REVIEW->"Use text";VoicePhase.ERROR->"Retry mic";else->"Finish" };else->"Select" }
        val x=when { compose->"Save draft";assistant->"Chats";remote->"Exact keys";else->"Actions" }
        val y=when { home->"Assistant";assistant->"Write";terminal || compose->"Write / mic";voice->"Keyboard";else->"—" }
        return listOf(
            ControllerCommand(LogicalAction.CONFIRM,"A",confirm,hint=true,enabled=!acquiring && !(voice && voicePhase in setOf(VoicePhase.TRANSCRIBING,VoicePhase.PERMISSION))),
            ...
            ControllerCommand(LogicalAction.COMPOSE,"Y",y,"Tap: ${if(terminal || compose)"write"else y} · hold: ${if(terminal || compose || voice)"dictation"else "assistant mic"}",hint=home || terminal || compose || assistant || voice,enabled=home || terminal || compose || assistant || voice),
